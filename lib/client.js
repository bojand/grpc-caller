...
function createClient (clientProto) {
  class GRPCCaller {
    ...
    withDefault(type, object={}) {
      return _.merge({}, this.defaults[type], object);
    }
  }
    ...
    if (typeof clientProto[k] === 'function') {
      if (!v.requestStream && !v.responseStream) {
        targetProto[k] = function (arg, metadata, options, fn) {
          ...
          options = this.withDefault('options', options)

          if (!metadata || !(isGRPCMetadata(metadata))) {
            metadata = create(this.withDefault('metadata', metadata))
          }
        ...
        targetProto[k] = function (arg, metadata, options) {
          ...
          if (!metadata || metadata instanceof grpc.Metadata === false) {
            metadata = create(this.withDefault('metadata', metadata))
          }
        ...
        targetProto[k] = function (metadata, options) {
          options = create(this.withDefault('options', options))
...
function createExec (clientProto) {
  clientProto.exec = function exec (request, fn) {
    ...
    const options = this.withDefault('options', request.options)
    const metadata = this.withDefault('metadata', request.metadata)
