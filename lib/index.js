...
function caller (host, proto, name, options) {
  ...
  clientProto._exec = function (request, fn) {
    if (!this._impl[request.methodName]) {
      throw new Error(String.raw`Invalid method name ${request.methodName}`)
    }

    let metadata
    if (_.isPlainObject(request.metadata) && request.metadata instanceof grpc.Metadata === false) {
      metadata = create(request.metadata)
    }

    const clientFn = this._impl[request.methodName].fn
    
    if (!fn.responseStream && !fn.requestStream) {
      const args = _.compact([arg, metadata, options, fn])
      // only promisify-call functions in simple response / request scenario
      return pc(this, clientFn, ...args)
    } else if (fn.responseStream && !fn.requestStream) {
      const args = _.compact([request.param, metadata, options])
