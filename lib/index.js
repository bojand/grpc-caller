 ...
 * @param {Object} defaults - Metadata and Options that are passed to the Request that the client generates
 * @returns {Object}
 *
 ...
 * 
 * @example <caption>Pass Default Metadata or Options</caption>
 ...
 *
 * // Now every call with that client will result in invoking the interceptor and sending the default metadata
 * 
 */
function caller (host, proto, name, credentials, options, defaults) {
  let Ctor
  ...
  const { metadata: defaultMetadata, options: defaultOptions } = Object.assign({}, defaults);

  return wrap(client, defaultMetadata, defaultOptions);
}

function wrap (client, metadata, options) {
